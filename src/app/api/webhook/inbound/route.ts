...
export async function POST(req: NextRequest) {
  try {
    ...
    const secrets = [
      ...
      process.env.WEBHOOK_SECRET_TILEDOODLING,
      process.env.WEBHOOK_SECRET_CHINESEINKCLASS,
      process.env.WEBHOOK_SECRET_ACRYLICCATCLASS,
      process.env.WEBHOOK_SECRET_BIBLEDOODLINGCLASS,
      process.env.WEBHOOK_SECRET_TREEOFLIFECLASS,
    ...
    if (matchedAccount && inserted) {
      after(() => runTriageForEmail(supabase, inserted as Email, matchedAccount, { allowAck: true, allowAlert: true }));
      if (emailId) {
        after(() =>
